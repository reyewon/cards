...
const questions = [
    ...
    { text: "What non-sexual act turns you on the most when we’re in a kink scene?", type: "D_asks_S" }, // Could be neutral, but often D directed
    ...
    { text: "What does aftercare usually look like for you? How can I best provide it?", type: "S_asks_D" }, // Doms need aftercare too
    { text: "What’s your favorite part of a power exchange dynamic with me?", type: "S_asks_D" },
    { text: "Do you prefer when I am rough with you or when I’m gentle?", type: "S_asks_D" }, // Asking Dom's preference for *their* experience
    { text: "What’s the most dominant thing you’ve done to me during a scene?", type: "S_asks_D" }, // Asking about Dom's action
    ...
    { text: "What mundane task would you make a sub do as punishment?", type: "S_asks_D" }, // Asking about Dom's ideas
    { text: "What role do you see yourself taking in a group scene? Dominant, submissive, or switch?", type: "S_asks_D" }, // Asking Dom's perspective
    { text: "What's a specific insecurity that ENM brings up for you, and how can I support you with it?", type: "S_asks_D" }, // Asking about Dom's feelings
    { text: "Describe a time you felt 'domspace'. What did it feel like?", type: "S_asks_D" },
    { text: "What's your favourite type of verbal interaction during a scene (praise, degradation, commands, silence)?", type: "S_asks_D" }, // Asking Dom's preference
    ...
    { text: "What’s your favorite way to tease me until I beg?", type: "S_asks_D" }, // Rephrased for S asking D
    { text: "When was the last time you pushed a boundary with me, and how did it feel afterward?", type: "S_asks_D" }, // Rephrased for S asking D
    { text: "What’s your favourite name to call me during a scene?", type: "S_asks_D" }, // Rephrased for S asking D
    { text: "Would it excite you to watch me being dominated by someone in a public or group setting?", type: "S_asks_D" }, // Rephrased for S asking D
    { text: "What's one thing you wish I did more of during our kink encounters?", type: "S_asks_D" }, // Rephrased for S asking D
...
function getUsedIndices(key) {
    try {
        ...
        // Ensure we return an array, even if stored value is null or invalid JSON
...
function getNextUniqueForfeit() {
    ...
    let attempts = 0;
    const maxAttempts = totalItems * 2; // Safety break for potential infinite loops
    let randomIndex;

    if (totalItems === 0) return "No forfeits available!"; // Handle empty list
...
function getNextUniqueQuestion() {
    ...
    if (askerRole === 'Dom' && answererRole === 'sub') {
        allowedTypes.push('D_asks_S');
    } else if (askerRole === 'sub' && answererRole === 'Dom') {
        allowedTypes.push('S_asks_D');
    } else if (askerRole === 'Dom' && answererRole === 'switch') {
        allowedTypes.push('D_asks_S'); // Treat switch as potentially submissive when asked by Dom
    } else if (askerRole === 'switch' && answererRole === 'Dom') {
        allowedTypes.push('S_asks_D'); // Treat switch as potentially submissive asking Dom
    } else if (askerRole === 'sub' && answererRole === 'switch') {
         allowedTypes.push('S_asks_D'); // Treat switch as potentially dominant when asked by sub
    } else if (askerRole === 'switch' && answererRole === 'sub') {
        allowedTypes.push('D_asks_S'); // Treat switch as potentially dominant asking sub
    }
    // Dom/Dom, sub/sub, switch/switch primarily use neutral
    ...
    if (availableIndices.length === 0) {
        ...
        if (usedQuestionIndices.length >= totalQuestions) {
            ...
            for (let i = 0; i < totalQuestions; i++) {
             ...
             // If still no questions (e.g., roles are D->S but only S->D questions exist)
             if (availableIndices.length === 0) {
                 console.warn("Post-reset fallback: No role-specific or neutral questions found. Picking any available question.");
                 // Pick from *any* question type
                 for (let i = 0; i < totalQuestions; i++) {
                     // No need to check usedIndices as it's empty
                     availableIndices.push(i);
                 }
             }
        ...
        if (availableIndices.length === 0) {
             console.error("Error: Could not find any available question index.");
...
function updateTurnIndicator() {
    ...
    const askerRole = (currentPlayer === 1) ? player1Role : player2Role;
    // Display roles for clarity
    turnIndicator.textContent = `${askerName} (${askerRole}) asks ${answererName}:`;
}
...
function displayNextForfeit() {
    ...
    const performer = (currentPlayer === 1) ? player2Name : player1Name;
    forfeitCard.textContent = `${performer}, your forfeit is: ${nextF}`;
